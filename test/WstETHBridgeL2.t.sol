...
import {CREATE3Factory} from "../src/helper/Create3Factory.sol";
...
contract WstETHBridgeL2Test is Test {
    ...
    function _getWstETHBridgeL2Address() internal returns (address) {
        return
            create3Factory.getDeployed(
                deployer,
                keccak256(bytes("WstETHBridgeL2"))
            );
    }
    ...
    function _getWstETHBridgeL1Address() internal returns (address) {
        return
            create3Factory.getDeployed(
                deployer,
                keccak256(bytes("WstETHBridgeL1"))
            );
    }
    ...
    function testUpgradeAsAdmin() public {
        ...
        WstETHBridgeL2V2Mock bridgeL2V2 = WstETHBridgeL2V2Mock(
            address(bridgeL2)
        );
